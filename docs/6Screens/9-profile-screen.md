...
The `ProfileScreen` will be responsible for displaying a users information in a user friendly manner. This infomation includes Name, Major, Profile Image, Connections, Portoflio, and Experience.
...
```
### useEffect Hooks
The constants `userName`, `userImage`, and `userMajor`are used to set the user information states to blank if data from the Firebase is hasn't been pulled yet.
...
```
## Styles
The component uses the `StyleSheet` object from react-native to define styles for Profile Informations containers, editing buttons, and bio container
### The styles object 
``` js
