...
The 'MessageScreen' handles all communication between all users of the application and has many messaging features included.  The database is handled via an open-source API called StreamChat.  Users can use the search function to look up users and create a group chat with one or multiple members.  
...
## Setting up Iniitial variables and states
```js
...
## Modals for selecting members to add to a group chat
...
```js
  ...
  ## Code creating a list of channels and having functions to create and open a group chat
